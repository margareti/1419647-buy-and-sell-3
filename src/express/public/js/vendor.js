...
!function (a) {
  let b = navigator.userAgent; a.HTMLPictureElement && /ecko/.test(b) && b.match(/rv\:(\d+)/) && RegExp.$1 < 45 && addEventListener(`resize`, function () {
    let b; let c = document.createElement(`source`); let d = function (a) {
      let b; let d; let e = a.parentNode; `PICTURE` === e.nodeName.toUpperCase() ? (b = c.cloneNode(), e.insertBefore(b, e.firstElementChild), setTimeout(function () {
        e.removeChild(b);
  ...
  "use strict"; function d(a) {
    ...
    let e; let f; let g; let h; return `saveData` === B.algorithm ? a > 2.7 ? h = c + 1 : (f = b - c, e = Math.pow(a - 0.6, 1.5), g = f * e, d && (g += 0.1 * e), h = a + g) : h = c > 1 ? Math.sqrt(a * b) : a, h > c;
  } function h(a) {
    let b; let c = s.getSet(a); let d = !1; `pending` !== c && (d = r, c && (b = s.setRes(c), s.applySetCandidate(b, a))), a[s.ns].evaled = d;
  } function i(a, b) {
    return a.res - b.res;
  } function j(a, b, c) {
    ...
    function c(b) {
      ...
      let a; let c; let d; let e; let f; let i; let j; let k; let l; let m = !1; let o = {}; for (e = 0; e < h.length; e++) {
        f = h[e], i = f[f.length - 1], j = f.substring(0, f.length - 1), k = parseInt(j, 10), l = parseFloat(j), X.test(j) && `w` === i ? ((a || c) && (m = !0), k === 0 ? m = !0 : a = k) : Y.test(j) && `x` === i ? ((a || c || d) && (m = !0), l < 0 ? m = !0 : c = l) : X.test(j) && `h` === i ? ((d || c) && (m = !0), k === 0 ? m = !0 : d = k) : m = !0;
    ...
    if (t) {
      let c; let d; let e; let f = a || {}; if (f.elements && f.elements.nodeType === 1 && (`IMG` === f.elements.nodeName.toUpperCase() ? f.elements = [f.elements] : (f.context = f.elements, f.elements = null)), c = f.elements || s.qsa(f.context || b, f.reevaluate || f.reselect ? s.sel : s.selShort), e = c.length) {
    ...
    let c; a.src = b.url, `image/svg+xml` === b.set.type && (c = a.style.width, a.style.width = a.offsetWidth + 1 + `px`, a.offsetWidth + 1 && (a.style.width = c));
  }, s.getSet = function (a) {
    ...
    let c; let d = a.attachEvent ? /d$|^c/ : /d$|^c|^i/; var e = function () {
      let a = b.readyState || ``; f = setTimeout(e, `loading` === a ? 200 : 999), b.body && (s.fillImgs(), c = c || d.test(a), c && clearTimeout(f));
    ...
    let b = a.shift(); `function` === typeof s[b] ? s[b].apply(s, a) : (B[b] = a[0], S && s.fillImgs({reselect: !0}));
  }}; for (;J && J.length;) {
